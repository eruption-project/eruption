...
pub fn bind_hiddev(
    ...
    if ctrl_dev.is_none() {
        ...
        Ok(Arc::new(RwLock::new(Box::new(RoccatKoneXp::bind(
            ctrl_dev.unwrap(),
        )))))
    }
}
...
impl DeviceInfoTrait for RoccatKoneXp {
...
impl MouseDeviceTrait for RoccatKoneXp {
    ...
    fn send_led_map(&mut self, led_map: &[RGBA]) -> Result<()> {
        ...
        if !self.is_bound {
            ...
            let buf: [u8; 122] = [
                ...
                (led_map[LED_11].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_11].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_11].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_12].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_12].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_12].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_13].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_13].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_13].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_14].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_14].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_14].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_15].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_15].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_15].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_16].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_16].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_16].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_17].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_17].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_17].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_18].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_18].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_18].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_19].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_19].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_19].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_20].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_20].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_20].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_21].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_21].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_21].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_22].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_22].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_22].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_23].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_23].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_23].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_24].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_24].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_24].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_25].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_25].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_25].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_26].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_26].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_26].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_27].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_27].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_27].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_28].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_28].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_28].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_29].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_29].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_29].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_30].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_30].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_30].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_31].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_31].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_31].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_32].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_32].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_32].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_33].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_33].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_33].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_34].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_34].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_34].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_35].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_35].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_35].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_36].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_36].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_36].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_37].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_37].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_37].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_38].r as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_38].g as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                (led_map[LED_38].b as f32 * (self.brightness as f32 / 100.0)).round() as u8,
                0xff,
                0xff,
                0xff,
            ];
