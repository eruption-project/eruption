    ...
    (i64) => {
        paste! {
            fn [<build_config_widget_ i64>] <F: Fn(i64) + 'static>(
                ...
                // scale widget
                // set constraints
                let mut adjustment = gtk::Adjustment::builder();
    ...
    ($t:ty) => {
        paste! {
            fn [<build_config_widget_color_ $t>] <F: Clone + Fn($t) + 'static>(
                ...
                let chooser = gtk::ColorButton::builder()
                    .rgba(&rgba)
                    .use_alpha(true)
                    .show_editor(true)
                    .build();
...
fn populate_visual_config_editor<P: AsRef<Path>>(builder: &Builder, profile: P) -> Result<()> {
    ...
    for manifest in profile.manifests.values() {
        ...
        let expander_container = gtk::Box::builder()
            .orientation(Orientation::Vertical)
            .homogeneous(false)
            .build();
            ...
            match data {
                Some(data) => {
                    ...
                    if let Err(e) = dbus_client::write_file(&path.as_ref(), &data) {
                        ...
                        let message_dialog = gtk::MessageDialog::builder()
                            .parent(&main_window)
...
pub fn initialize_profiles_page<A: IsA<gtk::Application>>(
    ...
    let id_column = gtk::TreeViewColumn::builder()
        .title("ID")
    ...
    let name_column = gtk::TreeViewColumn::builder()
        ...
        .sizing(TreeViewColumnSizing::Autosize)
        .build();
    let filename_column = gtk::TreeViewColumn::builder()
        .title("Filename")
        .sizing(TreeViewColumnSizing::Autosize)
        .build();
    let path_column = gtk::TreeViewColumn::builder()
        .visible(false)
        .title("Path")
        .build();
    ...
    gtk::prelude::TreeViewColumnExt::pack_start(&id_column, &cell_renderer_id, false);
    gtk::prelude::TreeViewColumnExt::pack_start(&name_column, &cell_renderer_name, true);
    gtk::prelude::TreeViewColumnExt::pack_start(&filename_column, &cell_renderer_filename, true);
